...
def test_end_to_end_full_flow_with_material_to_shopping(tmp_path, monkeypatch):
    ...
    # 2) Create an empty shopping list
    (data_dir / "shopping_list.json").write_text("[]", encoding="utf-8")
    # 3) Prepare a minimal service component: S1 uses material M0002
    comps = {
        "rows": [
            {
                "component_id": "C0001",
    ...
    # 4) Create the dialog and perform end-to-end steps
    app = _ensure_app()
    ...
    # Force material type selection and pick the first material
    type_widget = dialog.tbl.cellWidget(row, 0)
    if isinstance(type_widget, type(dialog.tbl).widget_type):
        pass
    # Use existing path to material selector
    dialog._update_name_widget(row, "material")
    name_widget = dialog.tbl.cellWidget(row, 2)
    if isinstance(name_widget, type(dialog.tbl).widget_type):
        try:
            name_widget.setCurrentIndex(1)
            dialog._on_material_selected(row, name_widget)
        except Exception:
            pass
    # Accept dialog to trigger add-to-shopping logic
    dialog.accept()
    # Verify shopping item added
